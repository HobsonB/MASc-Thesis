...
#Train multiple linear regression models and evaluate their accuracy accross all floors

#BEWARE IF YOU RUN THIS SCRIPT, YOUR COMPUTER WILL BE RUNNING CALCULATIONS FOR UP TO HALF AN HOUR
...
for (j in 1:length(combos)){
  ...
  for (i in 1:length(floordata.list)){
    ...
    #Train linear model
    mdl <- train(form = as.formula(paste("gt~", paste0(combos[j]))), data = as.data.frame(trains.list[i]), trControl=train_control, method="lm", tuneGrid=grid)
    
    for (k in 1:length(floordata.list)){
      
      if(k == i){
        predictand <- predict(mdl, as.data.frame(tests.list[i]))
      }else{
        predictand <- predict(mdl, as.data.frame(floordata.list[i]))
      }
...
write.csv(results.df,"mlr.predictions.csv")