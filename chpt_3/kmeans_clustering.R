...
#Performs k-means clustering on artificial occupancy data
...
set.seed()
ind <- as.numeric(kmeans(t(dat[c(7:21),]),nclust)$cluster)
...
if(should_plot == T){
  ...
  png(filename = "kmeans_clust.png", width = 2.75, height = 2.75, units = 'in', res = 600)
  ...
  text(x = 24, y = 485, pos = 2, labels = "k-means (c)", cex = 0.6)
  
  dev.off()
  
}

write.csv(ind, "kmeans_ind.csv", row.names = F)