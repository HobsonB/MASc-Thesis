...
#----Agglomerative Heirarchical Clustering Using Correlation Dissimilarity----

#Performs clustering using ACH 1-cor on artificial occupancy data
...
ind <- cutree(cor.hclust.mx,nclust)
...
if(should_plot == T){
  ...
  png(filename = "cor_clust.png", width = 2.75, height = 2.75, units = 'in', res = 600)
  ...
  text(x = 24, y = 485, pos = 2, labels = expression(paste("AHC, 1 - ", rho, " (a)")), cex = 0.6)
  
  dev.off()
  
}

write.csv(ind, "cor_ind.csv", row.names = F)